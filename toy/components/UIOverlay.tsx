...
import { Box, Bird, Cat, Rabbit, Users, Code2, Wand2, Hammer, FolderOpen, ChevronUp, FileJson, History, Play, Pause, Info, Wrench, Loader2, Music, Mic, Speaker, Disc, User, Keyboard, Drum, CassetteTape, Accessibility } from 'lucide-react';
...
interface UIOverlayProps {
  ...
  onDismantle: () => void;
  onRebuild: (type: 'Eagle' | 'Cat' | 'Rabbit' | 'Twins' | 'Guitar' | 'Microphone' | 'Speaker' | 'GoldRecord' | 'Singer' | 'Keyboard' | 'Drums' | 'Cassette' | 'Body') => void;
  onNewScene: (type: 'Eagle' | 'Cat' | 'Rabbit' | 'Twins' | 'Guitar' | 'Microphone' | 'Speaker' | 'GoldRecord' | 'Singer' | 'Keyboard' | 'Drums' | 'Cassette' | 'Body') => void;
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
        ...
        <div className="pointer-events-auto flex flex-col gap-2">
            ...
            >
                ...
                <DropdownItem onClick={() => onNewScene('Guitar')} icon={<Music size={16}/>} label="Guitar" />
                <DropdownItem onClick={() => onNewScene('Microphone')} icon={<Mic size={16}/>} label="Microphone" />
                <DropdownItem onClick={() => onNewScene('Speaker')} icon={<Speaker size={16}/>} label="Speaker" />
                <DropdownItem onClick={() => onNewScene('GoldRecord')} icon={<Disc size={16}/>} label="Gold Record" />
                <DropdownItem onClick={() => onNewScene('Singer')} icon={<User size={16}/>} label="Singer" />
                <DropdownItem onClick={() => onNewScene('Keyboard')} icon={<Keyboard size={16}/>} label="Keyboard" />
                <DropdownItem onClick={() => onNewScene('Drums')} icon={<Drum size={16}/>} label="Drums" />
                <DropdownItem onClick={() => onNewScene('Cassette')} icon={<CassetteTape size={16}/>} label="Cassette" />
                <DropdownItem onClick={() => onNewScene('Body')} icon={<Accessibility size={16}/>} label="Body" />
                <div className="h-px bg-slate-100 my-1" />
                <DropdownItem onClick={onPromptCreate} icon={<Wand2 size={16}/>} label="New AI build" highlight />
                <div className="h-px bg-slate-100 my-1" />
            ...
            {isDismantling && !isGenerating && (
                <div className="flex items-end gap-4 animate-in slide-in-from-bottom-10 fade-in duration-300">
                     ...
                     >
                        ...
                        <DropdownItem onClick={() => onRebuild('Guitar')} icon={<Music size={18}/>} label="Guitar" />
                        <DropdownItem onClick={() => onRebuild('Microphone')} icon={<Mic size={18}/>} label="Microphone" />
                        <DropdownItem onClick={() => onRebuild('Speaker')} icon={<Speaker size={18}/>} label="Speaker" />
                        <DropdownItem onClick={() => onRebuild('GoldRecord')} icon={<Disc size={18}/>} label="Gold Record" />
                        <DropdownItem onClick={() => onRebuild('Singer')} icon={<User size={18}/>} label="Singer" />
                        <DropdownItem onClick={() => onRebuild('Keyboard')} icon={<Keyboard size={18}/>} label="Keyboard" />
                        <DropdownItem onClick={() => onRebuild('Drums')} icon={<Drum size={18}/>} label="Drums" />
                        <DropdownItem onClick={() => onRebuild('Cassette')} icon={<CassetteTape size={18}/>} label="Cassette" />
                        <DropdownItem onClick={() => onRebuild('Body')} icon={<Accessibility size={18}/>} label="Body" />
                        <div className="h-px bg-slate-100 my-1" />
